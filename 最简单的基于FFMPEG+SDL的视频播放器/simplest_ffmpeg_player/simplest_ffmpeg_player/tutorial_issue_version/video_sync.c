...
/* 获取真实的audio_clock, 类似现在播放到的AVFrame的pts
 * 在audio_decode_frame里面累加了所有audio_buf数据
 * 实际使用的时候应该减掉剩余部分
 * */
...
static int synchronize_audio(VideoState *is, short *samples,int samples_size, double pts) {
    ...
    if(is->av_sync_type != AV_SYNC_AUDIO_MASTER) {
        ...
        ref_clock = get_master_clock(is); // video clock
        diff = get_audio_clock(is) - ref_clock;

        if(diff < AV_NOSYNC_THRESHOLD) { // diff在范围内
            // accumulate the diffs
            is->audio_diff_cum = diff + is->audio_diff_avg_coef * is->audio_diff_cum;

            if(is->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) { // 没有累计足够的diff
                is->audio_diff_avg_count++;
            } else {
                // 计算某段时间内的avg_diff
                avg_diff = is->audio_diff_cum * (1.0 - is->audio_diff_avg_coef);

                if(fabs(avg_diff) >= is->audio_diff_threshold) { //超过阈值 需要同步
                    ...
                    if(wanted_size < samples_size) {// 较少采样
                        /* remove samples */
                        samples_size = wanted_size;
                    } else if(wanted_size > samples_size) {// 增加采样, 一直复制最后一个采样
        ...
        if (is->pictq_size == 0) {
            ...
            if(is->av_sync_type != AV_SYNC_VIDEO_MASTER) {
                ...
                if (fabs(diff) < AV_NOSYNC_THRESHOLD) {
                    if (diff <= -sync_threshold) {// 视频落后,赶紧刷新
                        delay = 0;
                    } else if (diff >= sync_threshold) {// 视频领先, 延缓下一帧
                        delay = 2 * delay;
                    }
                }
            }
...
static double synchronize_video(VideoState *is, AVFrame *src_frame, double pts) {
    ...
    fps = av_q2d(is->video_st->r_frame_rate);

    /* if we are repeating a frame, adjust clock accordingly */
    // extra_delay = repeat_pict / (2*fps)
    // frame_delay += src_frame->repeat_pict * (frame_delay * 0.5);
    frame_delay = src_frame->repeat_pict / (2 * fps);
...
int our_get_buffer(struct AVCodecContext *c, AVFrame *pic,int flags) {
    int ret = avcodec_default_get_buffer2(c,pic,flags); //avcodec_default_get_buffer(c, pic);
...
static int video_thread(void *arg) {
    ...
    for (;;) {
        ...
        // packet->pts ---> global_video_pkt_pts --> AVFrame->opaque --> pts
        global_video_pkt_pts = packet->pts;


        // Decode video frame
        //任何时候当一个包开 始一帧的时候，avcodec_decode_video() 将调用一个函数来为一帧申请一个缓冲。
...
int play_media_with_videosync(char* url){
